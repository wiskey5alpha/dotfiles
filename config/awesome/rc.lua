...
-- set theme_name to the user theme directory
local theme_name = "aldrich"
local theme_path = require("gears.filesystem").get_dir("config") .. "themes/" .. theme_name .. "/theme.lua"
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
          layout = wibox.layout.fixed.horizontal,
            spotify_widget({
                font = 'Ubuntu Mono 9',
                play_icon = '/usr/share/icons/Papirus-Dark/16x16/actions/player_play.svg',
                pause_icon = '/usr/share/icons/Papirus-Dark/16x16/actions/player_pause.svg',
            }),
            mykeyboardlayout,
            volumebar_widget({
                  main_color = '#556B2F',
...
globalkeys = gears.table.join(
    ...
    -- modify the default mod-w to mod-shift-W so that i can use rofi
    ...
    -- Added for spotify shell
    awful.key({ modkey,         }, "d", function () spotify_shell.launch() end, {description = "spotify shell", group = "music"}),
    -- Modified the default "run prompt" to Mod-Shift-R so i can use Mod-R for rofi
...
client.connect_signal("manage", function(c)
                        ...
                        elseif lower_icon ~= nil then
                          local new_lower_icon = gears.surface(lower_icon)
                          c.icon = new_lower_icon._native

                          --Check if the client already has an icon. If not, give it a default.
                        elseif c.icon == nil then
                          local new_default_icon = gears.surface(menubar.utils.lookup_icon("application-default-icon"))
                          c.icon = new_default_icon._native
                        end
end)
